...
/*void chatbot(GtkButton *button,gpointer user_data){
...
//espeak -ven-us+f3 -a 100 for female voices in espeak
...
{ char command[]="espeak ";
  ...
  { fgets(array,200,fp);
     ...
     { flag*=0;
        fgets(array,200,fp); 
        array_length=strlen(array);
      	gtk_label_set_text(label,array);
        removespaces(array,array_length);
        strcat(command,array);
        system(command);
 ...
 if(flag==1)
 {
  strcat(command,"I_am_not_intelligent_enough_to_answer_it");
  system(command);
  gtk_label_set_text(label,"I am not intelligent enough to answer it");
  }
...
{ GtkBuilder *builder;
  ...
  gtk_init(&argc,&argv);
  	...
  	gtk_builder_add_from_file(builder,"speech.glade",NULL);
 	window=GTK_WIDGET(gtk_builder_get_object(builder,"window_main"));
 	button=GTK_BUTTON(gtk_builder_get_object(builder,"submit"));
        entry=GTK_ENTRY(gtk_builder_get_object(builder,"entry"));
  	label=GTK_LABEL(gtk_builder_get_object(builder,"output"));
    ...
    void on_submit_clicked(GtkButton *button,gpointer data)
    { 
      char exit[]="espeak ";
      int stringlength;
      char string[200];
      //printf("Getting data from the user:\n");
      strcpy(string,gtk_entry_get_text(GTK_ENTRY(data)));
      stringlength=strlen(string);
      upper(string,stringlength);
      if(strcmp(string,"EXIT")==0)
        {  strcat(exit,"Goodbye");
           system(exit);
    	   gtk_label_set_text(label,"Goodbye");       
           gtk_main_quit();}
    ...
    void quit_function(GtkButton *Button,gpointer data)
	{  
	char exit[]="espeak ";
    	char speak[]="Goodbye";
    	strcat(exit,speak);
